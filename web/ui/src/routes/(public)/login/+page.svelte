...
<svelte:head>
	<title>{$_('meta.loginTitle')}</title>
	<meta name="description" content={$_('meta.loginDesc')} />
</svelte:head>
...
>
	<div class="-m-px rounded-[calc(var(--radius)+.125rem)] border bg-card p-8 pb-6">
		<div class="text-center">
			...
			<h1 class="mt-4 mb-1 text-xl font-semibold">{$_('auth.signIn')}</h1>
			<p class="text-sm">{$_('auth.welcome')}</p>
		</div>
		...
		{#if oidcStatus.isSuccess}
			{#if oidcStatus.data.loginEnabled}
				<div class="mt-6 space-y-5">
					<div class="space-y-2">
						<Label for="username" class="block text-sm">{$_('auth.username')}</Label>
						<Input id="username" bind:value={username} {onkeydown} />
					</div>

					<div class="space-y-0.5">
						<Label for="pwd" class="text-title text-sm">{$_('auth.password')}</Label>
						<PasswordInput bind:value={password} {onkeydown} />
					</div>

					<Button class="w-full" type="submit" onclick={handleLogin}>{$_('auth.signInButton')}</Button>
				</div>
			{/if}
			...
			{#if oidcStatus.data.loginEnabled && oidcStatus.data.oidcEnabled}
				<div class="my-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
					...
					<span class="text-xs text-muted-foreground">{$_('auth.orContinueWith')}</span>
					<hr class="border-dashed" />
				</div>
			{/if}
			...
			{#if oidcStatus.data.oidcEnabled}
				<div class="flex flex-col gap-4">
					<Button variant="outline" onclick={handleOIDCLogin}>
						{$_('auth.loginWith', { values: { provider: oidcStatus.data.provider || 'OIDC' } })}
					</Button>
				</div>
			{/if}
		{/if}
